...
def p_bar(bar, symbols):
    ...
    bars_1h = symbols['BTCUSDT'].latest_1h(400)
    if bars_1h[-1].ts > last_c_1h:
        ...
        if middle >= stdev:
            if nt_macd_up:
                ...
                wechat_push(f'BTCUSDT up {bar.c:.2f}')
                nt_macd_up = True
        elif middle <= -stdev:
            if nt_macd_down:
                pass
            else:
                wechat_push(f'BTCUSDT down {bar.c:.2f}')
                nt_macd_down = True
        else:
            if nt_macd_up:
                wechat_push(f'BTCUSDT not up {bar.c:.2f}')
                nt_macd_up = False
            if nt_macd_down:
                wechat_push(f'BTCUSDT not down {bar.c:.2f}')
                nt_macd_down = False