...
line_hist = [line_sell[i] - line_buy[i] for i in range(len(line_sell))]
# ====== 绘图 ======
_min_val = min(min(line_buy), min(line_sell), min(line_p))
_max_val = max(max(line_sell), max(line_buy), max(line_p))

plot_line_with_axes(line_buy, _min_val, _max_val, color='green')
plot_line_with_axes(line_p, _min_val, _max_val, color='blue')
plot_line_with_axes(line_sell, _min_val, _max_val, color='red')
h_min = min(line_hist)
h_max = max(line_hist)
